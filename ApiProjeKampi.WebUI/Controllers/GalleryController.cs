using ApiProjeKampi.WebUI.Dtos.ImagesDtos;
...
namespace ApiProjeKampi_WebUI.Controllers
{
    public class GalleryController : Controller
    {
        ...
        public async Task<IActionResult> ImageList()
        {
            ...
            var responseMessage = await client.GetAsync("https://localhost:7156/api/Images/");
        ...
        public async Task<IActionResult> CreateImage(CreateImagesDto createImageDto)
        {
            ...
            var responseMessage = await client.PostAsync("https://localhost:7156/api/Images/", stringContent);
            if (responseMessage.IsSuccessStatusCode)
            {
        ...
        {
            var client = _httpClientFactory.CreateClient();
            var responseMessage = await client.DeleteAsync($"https://localhost:7156/api/Images?id={id}");
            return RedirectToAction("ImageList");
        }
        [HttpGet]
        ...
        {
            var client = _httpClientFactory.CreateClient();
            var responseMessage = await client.GetAsync($"https://localhost:7156/api/Images/GetImage?id={id}");
            if (responseMessage.IsSuccessStatusCode)
            {
                var jsonData = await responseMessage.Content.ReadAsStringAsync();
        ...
        public async Task<IActionResult> UpdateImage(UpdateImageDto updateImageDto)
        {
            ...
            StringContent stringContent = new StringContent(jsonData, System.Text.Encoding.UTF8, "application/json");
            var responseMessage = await client.PutAsync("https://localhost:7156/api/Images/", stringContent);
            if (responseMessage.IsSuccessStatusCode)
            {
                return RedirectToAction("ImageList");
            }
